    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <Link href="/" className="flex items-center gap-2 text-white font-bold text-xl tracking-wide">
          {site.logo ? (
            <img src={publicAssetUrl(site.logo)} alt={site.site_name || 'Logo'} className="w-8 h-8 object-contain rounded" />
          ) : (
          ...
          <span className="max-w-[180px] truncate">{site.site_name || 'TORTUGA AREA'}</span>
        </Link>

        {/* LINKS - desktop */}
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map(({ label, href }) => (
            <Link key={href} href={href} className={`${linkClass} ${pathname === href ? 'text-white' : ''}`}>
              {label}
              <span className={`absolute -bottom-1 left-0 h-[1.5px] bg-amber-400 transition-all duration-300 group-hover:w-full rounded-full ${pathname === href ? 'w-full' : 'w-0'}`} />
            </Link>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          <a
            href="/order"
            className="px-5 py-2 border border-white/25 hover:border-amber-400 text-white text-sm font-semibold rounded-full transition-all duration-300 hover:scale-105"
          >
            Pesan Menu
          </a>
          <a
      ...
      {menuOpen && (
        <div className="md:hidden px-6 pb-6 flex flex-col gap-4">
          ...
          <a
            href="/order"
            onClick={() => setMenuOpen(false)}
            className="w-full text-center px-5 py-3 text-white text-sm font-semibold border border-white/20 rounded-full transition-colors"
          >
            Pesan Menu
          </a>
          <a
